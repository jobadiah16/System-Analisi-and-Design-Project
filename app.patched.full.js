/* app.patched.full.js — Klinik Sentosa (Original + Non-invasive Patches + Unified Login)
   Versi ini:
   ...
   - memperbaiki masalah "harus refresh untuk login" dengan memasang satu handler login
     yang andal pada DOMContentLoaded.
   NOTE: blok login lama DIKOMENTARI sebagai backup — tidak dihapus.
*/
...
/* ---------- UNIFIED login handler (attach once on DOMContentLoaded) ----------
   This replaces the earlier approach that sometimes registered listeners too early.
   The old handler blocks are left commented below as backup.
*/
function attachLoginHandler() {
  // ensure DOM nodes are cached (should be set by DOMContentLoaded init)
  loginForm = loginForm || $('#login-form');
  loginError = loginError || $('#login-error');
  loginScreen = loginScreen || $('#login-screen');
  mainScreen = mainScreen || $('#main-screen');

  if (!loginForm) {
    console.warn('[attachLoginHandler] #login-form not found — login will not be bound.');
    return;
  }

  // remove any previous listener to avoid duplicate bindings in edge cases
  try {
    loginForm.removeEventListener('submit', window.__ks_login_handler_ref);
  } catch (e) {}
  ...
  const handler = (e) => {
    ...
    // If role mismatch, guide user and *do not* force refresh
    ...
    // load global & role data, prepare dashboard immediately
    loadGlobalPatients();
    ...
    // dispatch login events once (guarded) so other patches (TTS, prescriptions) know about it
    setTimeout(() => {
      const detail = { name: user.name || user.username, role: user.role, username: user.username };
      try {
        // always dispatch both document & window variants for compatibility
        if (!window.__ks_voice_notified) {
          ...
          // still emit a plain event for other listeners if they expect multiple logins in one session
          document.dispatchEvent(new CustomEvent('ks-login-success', { detail }));
        }
  ...
  // keep a ref so we can remove it later if needed
  window.__ks_login_handler_ref = handler;
  loginForm.addEventListener('submit', handler);
}

/* ---------- COMMENTED: old login handler blocks (kept as backup, do not remove) ----------
   The original file had two variants: one immediate binding and one fallback on DOMContentLoaded.
   Those could cause race conditions where the first binding ran before DOM nodes existed.
   They are intentionally left here commented so you still have the old logic for reference.
*/
...
function getAllMedicalRecordsForPatient(patientId) {
  const roles = ['Petugas Administrasi','Dokter','Perawat','Apoteker','Kasir','Manajer Klinik','Pasien'];
  let results = [];
  roles.forEach(r => {
    try {
...
document.addEventListener('DOMContentLoaded', ()=> {
  ...
  // Attach the unified login handler now that DOM is ready
  attachLoginHandler();
});
